...
class HashBasedAlgorithmXTest < Minitest::Test
  ...
  def test_with_multiple_solutions
    ...
    assert_equal [[:Z, :Y, :X, :V], [:X, :Y, :Z, :W]], x.solve
  end
  ...
  def non_deterministic(algo)
    algo.behaviors = algo.behaviors.merge(HashBasedAlgorithmX::NON_DETERMINISTIC)
    algo
  end

  def all_solutions(algo)
    algo.behaviors = algo.behaviors.merge(HashBasedAlgorithmX::ALL_SOLUTIONS)
    algo
  end

  def algorithm_x(*rows)
    matrix = HashBasedMatrix.new
    ...
    HashBasedAlgorithmX.new(matrix)
  end
end
